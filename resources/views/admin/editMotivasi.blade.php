...
@section('page_title', 'Edit Motivasi 🎨')
@section('page_subtitle', 'Sempurnakan konten inspiratif Anda')
    ...
    <div class="max-w-2xl">
        ...
        @if ($errors->any())
            <div
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <h4 class="text-sm font-bold text-rose-700 dark:text-rose-400 mb-1">Update Gagal</h4>
        ...
        <div class="card-premium rounded-[2rem] p-8">
            <form action="{{ route('motivasi.update', $motivasi->id) }}" method="POST" enctype="multipart/form-data"
                class="space-y-6" id="motivasiForm">
                @csrf
                ...
                <div>
                    ...
                    <select name="tipe" id="tipe"
                        ...
                        required>
                        <option value="text" {{ (old('tipe', $motivasi->tipe)) == 'text' ? 'selected' : '' }}>Teks Inspiratif
                        </option>
                        <option value="image" {{ (old('tipe', $motivasi->tipe)) == 'image' ? 'selected' : '' }}>Visual
                            (Gambar)</option>
                    </select>
                </div>

                {{-- Input Text --}}
                <div id="input-text" class="{{ (old('tipe', $motivasi->tipe)) == 'image' ? 'hidden' : '' }}">
                    ...
                    <textarea name="isi" id="isi" rows="4"
                        class="w-full bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-medium text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all outline-none @error('isi') !border-rose-400 dark:!border-rose-500 ring-2 ring-rose-400/20 @enderror">{{ old('isi', $motivasi->isi) }}</textarea>
                    @error('isi')
                ...
                {{-- Input Foto --}}
                <div id="input-foto" class="{{ (old('tipe', $motivasi->tipe)) == 'text' ? 'hidden' : '' }}">

                    {{-- Current Image Display (Read Only) --}}
                    @if($motivasi->foto)
                        <div
                            class="mb-6 p-4 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-100 dark:border-white/5">
                            <label class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">Visual Saat
                                Ini</label>
                            <div class="relative w-full aspect-[2/1] max-w-sm rounded-xl overflow-hidden shadow-sm group">
                                <img src="{{ asset('storage/' . $motivasi->foto) }}" alt="Preview"
                                    class="w-full h-full object-cover">
                                <div class="absolute inset-0 bg-black/10"></div>
                            </div>
                            <p class="text-[10px] text-slate-400 mt-2 italic">Foto ini akan tetap digunakan jika Anda tidak
                                mengupload foto baru.</p>
                        </div>
                    @endif

                    {{-- Upload New Image --}}
                    <label for="foto" class="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2">
                        {{ $motivasi->foto ? 'Ganti Visual (Opsional)' : 'Upload Visual Baru' }}
                    </label>
                    ...
                    <div class="relative group cursor-pointer mb-4">
                        ...
                        <div
                            class="w-full bg-slate-50 dark:bg-white/5 border-2 border-dashed border-slate-200 dark:border-white/10 rounded-2xl px-6 py-8 transition-all group-hover:border-primary-500 group-hover:bg-primary-50 dark:group-hover:bg-primary-900/10 text-center space-y-3">
                            <span
                                class="material-icons-round text-3xl text-slate-400 group-hover:text-primary-500 transition-colors">add_photo_alternate</span>
                            <div>
                                <h3 class="text-sm font-bold text-slate-700 dark:text-slate-300">Klik untuk
                                    {{ $motivasi->foto ? 'mengganti' : 'upload' }} foto</h3>
                                <p class="text-[10px] text-slate-400 mt-1">Hanya format JPG, PNG, WebP (Maks 6MB)</p>
                            </div>
                        </div>
                    </div>
                    @error('foto')
                    ...
                    {{-- New Upload Preview (Hidden by default) --}}
                    <div id="image-preview-container" class="hidden mt-4 animate-fade-in relative group/preview">
                        <p class="text-xs font-bold text-primary-600 mb-2 flex items-center gap-1">
                            <span class="material-icons-round text-sm">new_releases</span> Preview Upload Baru:
                        </p>
                        <div
                            class="relative w-full aspect-[2/1] rounded-2xl overflow-hidden border-2 border-primary-500 shadow-lg shadow-primary-500/10">
                            ...
                            <div
                                ...
                                <button type="button" onclick="retakeCrop()" id="btn-retake"
                                    ...
                                    <span class="material-icons-round text-sm">crop</span> Crop Ulang
                                </button>
                                <button type="button" onclick="cancelChange()" id="btn-cancel"
                                    ...
                                    <span class="material-icons-round text-sm">undo</span> Batal
                                </button>
                            </div>
                        </div>
                        <p class="text-[10px] text-center text-slate-400 mt-2 italic">
                            *Foto baru ini akan menggantikan foto lama setelah disimpan
                        </p>
                    </div>
                </div>
                ...
                <div class="pt-6 border-t border-slate-100 dark:border-white/5 flex justify-end gap-3">
                    <a href="{{ route('motivasi') }}"
                        class="px-6 py-3.5 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-500 dark:text-slate-400 rounded-xl font-bold text-sm transition-all">
                        Batal
                    </a>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            window.cropImage = function () {
                ...
                canvas.toBlob(function (blob) {
                    ...
                    previewContainer.classList.remove('hidden'); // SHOW ONLY NOW

                    const Toast = Swal.mixin({
                        toast: true, position: 'top-end', showConfirmButton: false, timer: 3000, timerProgressBar: true
                    });
                    Toast.fire({ icon: 'success', title: 'Siap disimpan!' });
            ...
            window.cancelChange = function () {
                // Cancel the new upload, revert to initial state
                fileInput.value = '';
                previewImg.src = '#';
